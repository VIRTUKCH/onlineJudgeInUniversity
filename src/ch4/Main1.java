...
//		Menuboard.boards = new Menu[Menuboard.menus.length]; // 1. Menu 배열을 생성
//		for (int index = 0; index < Menuboard.boards.length; index++) {
//			Menuboard.boards[index] = new Menu(index + 1, menus[index], prices[index]); // 2. Menu 배열에 메모리를 할당
//
//			// 생성자를 통해서 객체를 생성하기 바란다.
////			Menuboard.boards[index].setId(index + 1);
////			Menuboard.boards[index].setName(menus[index]);
////			Menuboard.boards[index].setPrice(prices[index]);
//		}
////		Menuboard m = new Menuboard();
//		return new Menuboard();
//		/*
//		 * 결국 return은 Menuboard Type으로 해야해. 그러면 어떻게 해야할까? 내가 지금 반환할 수 있는 Menuboard가 있나?
//		 * -> 없음 -> 내가 만들어서 반환해야 함 그러면 Menuboard를 대충 만들고 반환하고 나서 Menu클래스의 print()메서드로 다
//		 * 찍어도 되긴 함 하지만 그러면 의미가 떨어지니까, 가능하면 Menuboard.menus와 Menuboard.prices를 활용하자 그렇기
//		 * 위해서는 static 객체로 Menu[] 배열을 만들어야 하는데, 그 것도 static이어야 한다. 결국 그렇게 객체를 만들고 값을
//		 * 집어넣고 나서, Menuboard를 만들어서 반환하게 만들었다. 그런데, 이 과정에서 굳이 Menuboard를 반환해야 하는 이유는
//		 * 무엇이었을까?
//		 * 
...
//		Menuboard mboard = Menuboard.makeBoard(); // makeBoard는 Static메서드 + Menuboard를 반환해야 함. + private 생성자
//		mboard.print(); //
//	}
//}