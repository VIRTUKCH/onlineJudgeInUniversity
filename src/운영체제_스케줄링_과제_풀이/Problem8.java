...
abstract class Scheduler {
	...
	// 이 함수를 구현하는 데에 있어서 내가 한 일은 딱히 없다.
	...
	// 각 스케줄링 알고리즘별로 이 함수를 재정의하되, 각 스케줄링 알고리즘의 재정의 함수에서 super클래스의 이 함수를 제일 먼저 호출해야 함
	public void clockInterrupt() {
		currentTime++; // 현재시간을 1 증가함 -> 매 시간 단위마다 호출되기 때문.
		...
		for (Process p; (p = jobs.getNewProcess(currentTime)) != null;) { // p는 새로 도착한 프로세스 객체
			isNewProcessArrived = true; // 새로운 프로세스가 오지 않으면 null이 아니게 되니까
...
class SPN extends Scheduler {
	...
	@Override
	public void schedule() {
		super.schedule(); // 이 메소드 구현 시 먼저 슈퍼 클래스의 schedule()를 먼저 호출

		Process minTimeProcess = new Process("minTimeProcess", 100, 100); // temp용

		// for문을 이용하여 readyQueue에 있는 프로세스들 중 서비스 시간이 가장 작은 프로세스를 찾는다.
		for (Process p : readyQueue) {
			if (p.getServiceTime() < minTimeProcess.getServiceTime() && p != minTimeProcess) {
				minTimeProcess = p;
			}
			currentProcess = minTimeProcess; // 찾은 프로세스를 currentProcess에 설정한다.
		}
...
// 6번 문제의 요구사항인 HRRN 스케줄링 클래스
class HRRN extends Scheduler {
	protected HRRN(String name) {
		super(name);
	} // 스케줄러 이름
	...
	public void schedule() {
		...
		// for문을 이용하여 readyQueue에 있는 프로세스들 중 가장 큰 값의 HRRN을 가지는 프로세스를 찾는다.
		for (Process p : readyQueue) {
			if (p.getResponeRatioTime(currentTime) > minTimeProcess.getResponeRatioTime(currentTime)
					&& p != minTimeProcess) {
				minTimeProcess = p;
			}
			currentProcess = minTimeProcess; // 찾은 프로세스를 currentProcess에 설정한다.
...
// 7번 문제의 요구사항인 SRT 스케줄링 클래스
class SRT extends Scheduler {
	protected SRT(String name) {
		super(name);
	} // 스케줄러 이름
...
//      for문을 이용하여 readyQueue에 있는 프로세스들 중 가장 작은 Remaining Time을 가지는 프로세스를 찾는다.
		for (Process p : readyQueue) {
			if (p.getRemainingTime() < minTimeProcess.getRemainingTime() && p != minTimeProcess) {
				minTimeProcess = p;
			}
			currentProcess = minTimeProcess; // 찾은 프로세스를 currentProcess에 설정한다.
		}
...
//  이 메소드 구현 시 (SRT는 선점형 스케줄러이므로, 기존에 실행하던 프로세스가 종료했거나 또는 새 프로세스가 도착하면 새로 스케줄링해야 한다.)
//  먼저 슈퍼 클래스의 isSchedulable()를 먼저 호출하여 스케줄링 가능한 경우 또는 새로 도착한 프로세스가 있는 경우 true를 반환
//	(리턴문을 한 문장으로 코딩해 보라)
	@Override
	public boolean isSchedulable() {
...
//        현재 execTime이 quantum과 동일한 경우 true 반환;
	...
	@Override
	public void schedule() {
		super.schedule(); // 먼저 슈퍼 클래스의 schedule()를 먼저 호출
		if (timeQuantumExausted()) { // 현재 실행하는 프로세스가 주어진 time quantum을 모두 소진한 경우(timeQuantumExausted())
			Process firstProcess = readyQueue.remove(0); // readyQueue의 맨 앞쪽 프로세스를 큐의 맨 끝으로 이동
			readyQueue.add(readyQueue.size(), firstProcess); // (즉, 맨 앞쪽 프로세스를 제거해서 큐의 맨 마지막에 추가함)
		}
		currentProcess = readyQueue.peek();
		execTime = 0;
	}
	...
	@Override
	public void clockInterrupt() {
		super.clockInterrupt(); // 먼저 슈퍼 클래스의 상응하는 메소드를 먼저 호출하고
		execTime++; // execTime을 1 증가함
	}
}
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			case 4:
				...
				break;
			case 5:
				cs.run(new HRRN("HRRN")); // 3번과 4번의 차이 : 일회용 객체
				break;
			case 6:
				cs.run(new SRT("SRT"));
				break;
